...
async function proxy(request: NextRequest, params: { path: string[] }) {
  ...
  const url  = `${API_URL}/sucursales/${ruta}${request.nextUrl.search}`;
...
export const PATCH  = async (req: NextRequest, ctx: any) => proxy(req, await ctx.params);
export const DELETE = async (req: NextRequest, ctx: any) => proxy(req, await ctx.params);
