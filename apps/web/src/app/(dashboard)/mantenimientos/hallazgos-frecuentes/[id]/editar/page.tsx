...
export default function PaginaEditarHallazgoFrecuente() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const { hallazgosFrecuentes, cargando, actualizar } = useHallazgosFrecuentes();
  const hallazgoFrecuente = hallazgosFrecuentes.find((h) => h.id === id);
  ...
  return (
    <div className="p-6 md:p-7.5">
      ...
      <div className="mx-auto max-w-[560px]">
        <h1 className="mb-4 text-heading-6 font-bold text-dark dark:text-white">Editar hallazgo frecuente</h1>
        {cargando ? (
          <p className="text-body-sm text-dark-4 dark:text-dark-6">Cargando...</p>
        ) : !hallazgoFrecuente ? (
          <p className="text-body-sm text-red">No se encontró el hallazgo frecuente.</p>
        ) : (
          <FormularioHallazgoFrecuente
            valoresIniciales={hallazgoFrecuente}
