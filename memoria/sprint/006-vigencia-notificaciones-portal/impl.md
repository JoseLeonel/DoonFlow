...
Documentadas después de implementar y verificar E2E — complementan (no reemplazan) el resto de este archivo, escrito antes de tocar código real.

1. **Bug real de HU-7 (escalamiento), encontrado en la verificación E2E**: la primera versión de `sp_accion_correctiva_escalar()` filtraba `WHERE estado = 'VENCIDO'`, siguiendo el `task.md`/`impl.md` originales (2026-07-16, escritos antes de 013). Pero el valor literal `'VENCIDO'` **nunca se persiste** en la columna `accion_correctiva.estado` — 013 lo calcula únicamente en lectura (`calcularEstadoEfectivo()` en `apps/api/src/modules/inspeccion/domain/accion-correctiva.entity.ts`, documentado ahí mismo: "es la única fuente de verdad de vencimiento... El campo `estado` persistido en BD puede quedar desactualizado"). El SP nunca encontraba ninguna fila y el escalamiento quedaba silenciosamente roto (sin error — simplemente 0 escaladas siempre, indistinguible de "no hay nada que escalar"). Detectado forzando una acción a `fecha_limite = now() - 10 days` vía `UPDATE` directo y confirmando que la primera versión del SP no la encontraba. **Corregido** a `estado NOT IN ('CUMPLIDO', 'NO_CUMPLIDO') AND fecha_limite < now() - interval '7 days'` (misma fórmula que `calcularEstadoEfectivo`). **Lección para sprints futuros**: cualquier SP que filtre por un estado que el dominio TypeScript calcula solo en lectura (no persistido) debe replicar la fórmula de cálculo completa, nunca asumir que el valor derivado existe como string en la columna.
2. **Rol `administrador_general` no existe**: igual que 011, el `task.md`/`impl.md` originales pedían notificar "al administrador_general de la empresa" como fallback de `sp_accion_correctiva_escalar()`. Ese rol no está en `ROLES_SISTEMA` — se usa `administrador` real.
3. **Sin `shared/jobs/programador.ts` genérico**: se creó `modules/notificaciones/infrastructure/job-notificaciones.job.ts` con su propio `cron.schedule()`, mismo patrón exacto que `modules/retencion/infrastructure/job-purgar-retencion.job.ts` (010) — no se introdujo una abstracción "programador" transversal que el resto del proyecto no usa. Ambos jobs (retención + notificaciones) corren en el mismo proceso `apps/api`, cada uno con su propia expresión cron (`RETENCION_JOB_CRON`, `NOTIFICACIONES_JOB_CRON`).
4. **Endpoint de historial de notificaciones sin `inspeccionId` para `accion_correctiva`/`hallazgo`**: el modelo `Notificacion` (definido en el `task.md` original) solo guarda `referenciaTipo`/`referenciaId` apuntando directo a la acción/hallazgo, sin el id de la certificación de origen. El deep-link exacto que pedía el diseño original ("navega a `/certificaciones/[inspeccionId]/plan` con foco en la acción") no es posible sin una consulta adicional. Se simplificó: `accion_correctiva` → `/certificaciones/seguimiento` (lista "Mis acciones", ya filtrada al usuario), `hallazgo` → `/certificaciones` (lista general); `inspeccion` sí navega directo porque su `referenciaId` **es** el id de la certificación.
5. **Server Component de `/verificar/[codigo]` no reutiliza el proxy Next.js**: llama directo a `API_URL` server-to-server (mismo patrón que cualquier fetch de servidor), no a `/api/verificacion/[codigo]` — ese proxy sigue existiendo y sirve para cualquier fetch futuro desde el cliente (ej. si `/verificar` pasara a ser un Client Component), pero un Server Component ya corre en el servidor y no necesita el salto adicional a través de su propio dominio.
6. **Sin tests en `packages/ui`**: `campana-notificaciones.tsx` no tiene test en `packages/ui/src/__tests__/` (como decía el `task.md` original) porque ese paquete nunca tuvo infraestructura de tests en ningún sprint previo (Boton, Breadcrumb, DialogoConfirmacion, Paginador, TablaComparativa tampoco la tienen). Se probó desde `apps/web` (que ya tiene vitest+RTL configurado) importando el componente real vía `@doonflow/ui`.
