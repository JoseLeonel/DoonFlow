...
## Desviaciones reales encontradas durante la implementación de HU-3 (2026-07-24)

1. **`claveHash` con SHA-256 + índice único, no bcrypt con escaneo lineal.** Este documento (arriba) diseñaba `claveHash` con bcrypt, verificado con `bcrypt.compare()` contra **cada** `ApiKey` de la BD hasta encontrar coincidencia (bcrypt no es determinista, no se puede indexar). Se implementó distinto y deliberadamente mejor: SHA-256 (determinista) con `clave_hash` **único e indexado**, lookup directo `WHERE clave_hash = sha256(claveRecibida)` en O(1) en vez de un escaneo O(n) en cada request a la API pública. La justificación de bcrypt (resistencia a fuerza bruta offline sobre contraseñas de baja entropía) no aplica aquí: una `ApiKey` ya es un secreto de 256 bits generado aleatoriamente, no una contraseña elegida por un humano — SHA-256 es el estándar de la industria para este caso (así lo hacen Stripe/GitHub con sus tokens de API). El propio documento original ya anticipaba esta alternativa como plan B "si el rendimiento lo exige" — se implementó directo como plan A, evitando el problema de escalabilidad reconocido desde el diseño.
2. **Prefijo de clave `dnf_live_`, no `dk_live_`** — identificable con la marca DoonFlow, cambio cosmético sin impacto funcional.
3. **`req.apiKey = { id, empresaId }`, no `req.empresaIdApiKey`** — se agrupó en un objeto (mismo patrón que `req.usuario`) para dejar espacio a más campos si se necesitan después (ej. `nombre` para logging/auditoría de uso de la API pública).
4. **Rate limit fijo en 60 req/min, no configurable vía `RATE_LIMIT_API_PUBLICA_POR_MINUTO`** — mismo criterio que el portal humano de 006 (`express-rate-limit` con límite hardcodeado), se puede extraer a variable de entorno después si se necesita ajustar sin redeploy.
5. **Sin restricción de `codigoVerificacion` por empresa dueña de la API key** — deviación de diseño consciente, no un gap: el portal humano de 006 (`VerificarCertificadoUseCase`, reutilizado tal cual) es totalmente anónimo y nunca filtra por empresa; restringir aquí violaría la regla 4 de la spec ("mismo subconjunto de datos que el portal humano"). Ver razonamiento completo en `checklist.md`.
6. **2 bugs reales encontrados y corregidos en la verificación E2E**: `claveHash` se filtraba en las respuestas JSON (`ApiKeyController` hacía spread directo del objeto de dominio) — corregido con una función `serializar()` que excluye el campo explícitamente. El botón "Revocar" del frontend ejecutaba la acción sin pedir confirmación — corregido usando `DialogoConfirmacion` de `packages/ui` (ya existía en el design system, solo no se había conectado a este flujo).
7. **Sin test de integración tipo supertest para el endpoint público** — el proyecto nunca adoptó esa infraestructura en ningún módulo (todos los sprints anteriores usan solo tests unitarios con mocks + verificación E2E manual vía curl); se mantuvo el mismo patrón en vez de introducir una dependencia nueva (`supertest`) solo para este endpoint.
8. **Efecto colateral fuera del alcance original de HU-3, a pedido explícito del usuario en la misma sesión**: se corrigieron los 225 errores preexistentes de `pnpm --filter api build` (`tsc` estricto), nunca antes ejecutado en el proyecto. Ver `memoria/estado.md` → bullet de sprint 009 y la memoria auto-persistente `project-sprint009-integraciones-resume` para el detalle completo (incluye 1 bug real de producción en `GestionarPlantillaUseCase.crear()` y 4 mocks de test desactualizados encontrados de paso).
